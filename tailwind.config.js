  ...
  theme: {
    extend: {
      ...
      keyframes: {
        ...
        // ── 召喚の"儀式"＝**待ち画面（GeneratingOverlay）が本体**。
        // 方向は `--a`（各要素にインラインで渡す角度）、開始時刻は animationDelay で散らす。
        // 画像アセットを使わない（たからばこの背景と同じ「アート依存を作らない」方針）。
        // ※完成後に光が集まって弾ける演出（summonGather/Flash/Ring/Emerge/Spark）は
        //   同じ絵を2回見せることになるので不採用にした（DECISIONS 2026-08-12）。
        // 生成中（召喚・合成）の"溜め"。光が外から中心へ**流れ続ける**。
        summonPull: {
