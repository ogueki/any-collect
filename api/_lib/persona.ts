...
/**
 * 会話に注入する接地文脈（v2・STEP2）。記憶（構造化ファクト）＋図鑑/アルバム傾向＋時間帯。
 * 会話は api/chat.ts でこの context を組み立てて buildSystemPrompt に渡す。
 *
 * ⚠️ **好感度（なつき度）は載せない**（2026-08-18）。レベルで口調を変えるのはやめた＝
 * 理由は DECISIONS。なつき度自体は残っているが、会話の文面には一切効かない。
 */
...
export function buildSystemPrompt(personaText: string, context?: ChatContext): string {
  const lines = [
    ...
    // ⚠️ **返答の長さ・絵文字・記法の指定はここが唯一の持ち主**（persona.md には書かない＝
    // 2か所に書くと必ず片方が腐る。キャラを差し替えても崩れてはいけない箇所なので、キャラ定義でなく契約側に置く）。
    '- 1回の返答は1〜3文。毎回同じ長さにせず揺らぎを持たせる（短い相づちにひとこと添えるだけの回も、乗ってきた3文の回もある）。機械のように均質な長さにしない。',
    '- 絵文字は使わない。',
