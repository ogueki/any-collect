 ...
 */
const ART_STYLE_BLOCK = `ART STYLE (MUST be identical for every item so the whole collection looks like one matching set):
- Cute chibi "kawaii" anime game-item icon, hand-drawn storybook look.
- One bold, clean, even dark outline around the whole item.
- Soft cel shading with 2-3 light steps, gentle top-left light source, soft ambient occlusion.
- Vibrant yet slightly pastel palette, glossy highlights, a tiny magical sparkle accent.

COMPOSITION (icon template, IDENTICAL every time):
- Exactly one single item, centered, slight 3/4 view, occupying about 75% of the frame.
- Square 1:1 framing.
- Background: a smooth soft pastel radial gradient inside a subtle rounded badge, very simple, no scenery, no environment.
- A soft drop shadow beneath the item.
- Absolutely NO text, letters, numbers, logos, watermarks, borders, UI, frames-with-text, or hands.`
...
export function buildItemImagePrompt(): string {
  return [
    'Transform the main real-world object in the provided photo into a single collectible fantasy game item icon.',
    'Keep the object recognizable (same kind of thing, its main colors and key features), but fully restyle it as a cute fantasy collectible — not a photo.',
    '',
    ART_STYLE_BLOCK,
  ].join('\n')
}
