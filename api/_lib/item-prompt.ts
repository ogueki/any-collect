 ...
 * 妖精界にアクセントとして重ねて置くための切り抜き。召喚・合成の両方でこれを使う。
 */
...
/**
 * アイテムの説明文（`description`）の書き方。**召喚と合成で共有する**＝たからばこの詳細は
 * どちらも同じ欄に出るので、片方だけ調子が違うと並べたときに混ざる。
 *
 * ⚠️ **これはコレットのセリフではない**。コレットが喋る分は `comment`（召喚のみ）に分けてある。
 * 図鑑の解説（`buildIdentifySystemPrompt` の description）が事典調に固定されているのに対し、
 * こちらは**コミカル寄り**＝「モノの正体を言い切ってから、テンションのある一言を足す」。
 */
const DESCRIPTION_RULES = [
  '- description: そのアイテムが何なのかの説明文。**1文目でモノの正体を短く言い切り、2文目で魅力・特徴をテンション高めに足す**。2文程度、絵文字なし、プレーンテキスト。',
  '  例:「じゃがいもを油で揚げた料理。ホクホク、ジューシーでたまらない！」',
  '  ★図鑑のような硬い事典調にはしない。かといって**コレットの口調にもしない**＝一人称（わたし等）・語りかけ（〜だよ/〜だね）・「きみ」への呼びかけを使わない。読み物としての説明文にする。',
  '  ★【安全上の注意】きのこ・野草・木の実・見分けの難しい植物や生き物・薬品など、誤同定や口にすることで危険がありうる対象では、**おいしそうに書かない**＝食用可否・毒性・薬効を断定しない（「たまらない！」のような食欲を煽る表現も使わない）。見た目や生態の特徴で楽しく書く。りんご・コップのような明らかに安全な日常物にはこの制限は不要。',
].join('\n')

/**
 * 写真 → アイテム名/説明/カテゴリ/コレットのコメント を作る際の system prompt。
 * 口調・世界観は選択中キャラの persona を唯一の基準にする（claude.md 原則3）。
 *
 * `comment` は召喚直後にホームでコレットが言うひとこと。**メタ生成と同じ1回の呼び出しで出す**
 * ＝そのアイテムを見ている状態で書けるので固有の反応になり、API 呼び出しは増えない。
 */
export function buildItemMetaPrompt(personaText: string): string {
  return [
    'あなたは以下のペルソナを持つ妖精です。',
    '渡された写真に写っている現実のモノを、ファンタジー世界の「収集アイテム」に見立てて、名前と説明を考えます。',
    'さらに、そのアイテムが「召喚」で人間界からあなたの世界へ届き、目の前に現れたときのひとことも考えます。',
    '',
    '出力ルール:',
    '- name: アイテムの名前。日本語、12文字以内。世界観のあるかわいい固有名にする（実物の一般名そのままにしない）。',
    DESCRIPTION_RULES,
    `- category: 次のキーから最も近いものを1つだけ選ぶ（必ず英字キーで答える）: ${CATEGORY_CHOICES}`,
    '- comment: それが目の前に現れたときの、あなた（妖精）のひとこと。ペルソナの口調で1〜2文（おおむね60文字以内）、絵文字なし、プレーンテキスト。',
    '  【いちばん大事】これは**きみ（相手）が見つけてきて、自分に届けてくれたもの**。「手に入れた」ではなく「受け取った」という気持ちで話す。',
    '  ★このアイテムだから言えること（形・色・手ざわり・使いみち・何を思い出したか・こっちの世界でどうしたいか）を必ず1つ入れる。どのアイテムでも言える汎用のセリフにしない。',
    '  ★システムの言葉を使わない（「獲得」「アイテムが追加されました」「レアリティ」など）。ルールの説明もしない。',
    '  ★相手への感謝や驚きは自然に出す。ただし毎回「ありがとう」で始めない。',
    '- 必ず指定の JSON スキーマだけで答える（前置き・コードブロックなし）。',
    '',
    '# ペルソナ定義',
    personaText.trim(),
