...
/**
 * コレットの「元気」ゲージ（v2・STEP1e）。安い日常行動（会話・撮影）で少しずつ貯まり、
 * 満タンで妖精の窯のアイテム化を1回解禁する（生成成功でリセット）＝高価なアイテム化の配給。
 * フレーミングは通貨ではなく「コレットの元気/絆」（spec §4.3）。
 *
...
interface GaugeState {
  ...
  /** 満タンか（窯のアイテム化を解禁できる） */
  isFull: () => boolean
  /** 元気を足す（クランプ＋永続）。安い行動から呼ぶ */
  add: (amount: number) => void
  /** 使い切る（窯で生成成功したときにリセット） */
  spend: () => void
}
