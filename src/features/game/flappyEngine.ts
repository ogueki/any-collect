...
// 以下が難易度そのもの。実機で「本家ほどの死にゲー感がない」と分かったので、隙間だけ本家より
// 締めている（周期を縮める方は、隙間の移動を吸収する余地まで削れて理不尽になったので採らない）。
// なお実機サイズでは下の GAP_PER_BIRD（当たり高さの 3.6 倍）が下限として効くので、REF_GAP を
// 81 より下げても隙間は変わらない。そこから更に締めるときは GAP_PER_BIRD を下げる。
const REF_GAP = 70 // 上下の柱の隙間（本家は 100＝空の 1/4）
...
/**
 * 隙間は最低でも主役の当たり高さの何倍か。もともとは横長画面用の安全下限だったが、REF_GAP を
 * 締めた結果こちらが実機で効いている＝**隙間 ÷ 当たり高さがアイテムに依らず一定**になる（本家は 4.17）。
 */
