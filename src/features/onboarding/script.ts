...
/**
 * 召喚コーチに添える**システムの補足**（Beat4）。コレットのセリフは気持ち（やってみたい）を言い、
 * 仕組み（何が条件で・何ができるのか）はこちらが説明する＝`CAMERA_HINT_TEXT` と同じ役割分担。
 * キャラにルール説明をさせないことで"中の人"感を出さない。声も演技指示も持たない（読み上げない）。
 */
export const SUMMON_COACH_NOTE =
  'コレットのまほうパワーが満タンの時、ずかんから1つ選んでコレットの世界に召喚できます。'
