...
### ③ 好感度レベル別（任意）
同じ感情でも好感度で見せる絵を変えたいときは、感情フォルダの中に
`lv1` `lv2` … のサブフォルダを作って入れます（lv は **1 始まり**）。
...
- 表示は現在のレベルに対し **`lv{level}` → 下位レベル → lvなし共通 → neutral** の順でフォールバック。
  だから **全レベル分の絵が揃ってなくても動く**（上位が無ければ下位が出る）。
- `lv` を一切作らない感情は、これまで通り全レベル共通で出ます。
- レベルの値（好感度の源）はアプリ側で決めます。未配線のうちは既定 lv1。
