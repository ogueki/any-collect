...
export interface ChatMessage {
  ...
  /**
   * **アプリ由来の発言**という印（未指定＝ホームで実際に交わされた会話）。
   * `camera`＝カメラでのコレットの反応／`album`・`zukan`＝「これの話をする」で振った定型文／
   * `summon`＝召喚したアイテムへのひとこと。
   * **付いているものは記憶の要約対象から外す**：
   * - camera … ファクト化すると毎回プロンプトに載り「前に撮ったもの」を機械的に蒸し返す
   * - album / zukan … 定型文なので「話題: 写真の話をしたい」のような無意味なファクトになる
   * - summon … 同上。1日1個の行為なので毎回ファクト化すると持ち物の羅列が記憶を埋める
   */
  origin?: 'camera' | 'album' | 'zukan' | 'summon'
  /**
   * この発言に紐づくアルバム写真の id（「この写真の話をする」で話題を振ったとき）。
   * 会話ログにサムネイルを出すためだけの参照。写真が削除されていたら黙って出さない。
   */
  photoId?: string
  /** 同上・図鑑エントリの id（「これの話をする」で話題を振ったとき）。 */
  entryId?: string
  /**
   * 同上・たからばこのアイテムの id（召喚したものへのひとこと）。
   * ホームでは**このセリフが大セリフである間だけ、立ち絵の横にそのアイテムを浮かべる**
   * ＝次の発言で自然に消えるので、消すための状態を持たなくてよい。
   */
  itemId?: string
}
