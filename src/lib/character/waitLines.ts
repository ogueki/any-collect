 ...
 * `synthesizing`（釜＝2アイテム合成の待ち）。どちらも GeneratingOverlay で使う。
 * ⚠️ 1つを呼び出す召喚と、2つを混ぜる釜では言うことが違う。コピーを共用しない。
 */
...
const STATUS_STAGES: Record<WaitContext, string[]> = {
  ...
  synthesizing: ['釜に火を入れてるよ…', 'ふたつを混ぜ合わせてるよ…', 'もうすぐできあがり…'],
}
...
const TIPS: Record<WaitContext, Record<string, string[]>> = {
  ...
  synthesizing: {
    default: [
      ...
      'ぐつぐつの具合がだいじなんだよ…って、わたしが見てるの',
      'おなじ組み合わせでも、毎回ちがうものができるかも',
      'どきどき…うまく混ざるといいね',
      'むかしの妖精は、釜でお星さまも作れたんだって',
    ],
  },
}
