...
## プロジェクト概要
妖精の相棒 **コレット** と、いつもの毎日を「ちょっと冒険」に変える寄り添いブラウザWEBアプリ。**カメラモード**（世界を見せる＝撮影→声つき反応→写真をアルバムに保存）と **ホームモード**（会話が主役・図鑑からの召喚魔法でアイテム化・たからばこ・メニュー＝窯で合成/アルバム/ゲーム）の2モード構成。
- **背骨**：コレットの"欲・好奇心"がユーザーを現実へ連れ出す（「行きたいなら行ってみるか」）。
- **二重構造**：カメラ→写真→**アルバム（あなたの世界）**／図鑑エントリ→召喚魔法→透過アイテム→**たからばこ（コレットの宝箱＝妖精界にある）**／召喚＝橋。妖精の窯＝アイテム2つの合成（メニュー内）。
- **v2 転換（2026-07-02）**：旧「撮影→即スキャン画像生成で収集」から、**会話＋アルバム＋アイテムの置き場**へ移行（コスト・リテンション両面）。詳細は `spec.md`、進行は `ROADMAP.md`。
- **たからばこ転換（2026-07-20・STEP5）**：アイテムの置き場を「妖精界＝作り込んだ風景シーン」から **「たからばこ＝4次元空間にアイテムが浮く収納ビュー」** に変更（背景はコード生成＝アート依存なし）。**妖精界は世界設定（正典）としては存続**＝`persona.md` は無改修。
